...
`sudo` -- but only entries whose ownership actually differs (a `find
\( ! -uid ! -gid \) -exec chown ... {} +`, not an unconditional `chown -R`),
since `USERNAME=root` -- the default for an autogenerated/unspecified
`remoteUser`, as in this repo's own `test-autogenerated` CI job -- is
already the owner of everything just installed, and a full recursive chown
would still walk the whole tree to confirm that. `USERNAME` follows the
