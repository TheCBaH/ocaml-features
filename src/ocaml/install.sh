...
# Only re-own entries whose ownership actually differs (e.g. USERNAME=root,
# the default for an autogenerated/unspecified remoteUser, is already the
# build-time owner of everything opam just installed) -- an unconditional
# `chown -R` still walks the whole tree even when it changes nothing.
# Mirrors ocaml-devcontainer's fix-opam-owner.sh, a sibling fix for the same
# waste in the unrelated case of a postCreate ownership correction.
chown_uid=$(id -u "$USERNAME")
chown_gid=$(id -g "$USERNAME")
